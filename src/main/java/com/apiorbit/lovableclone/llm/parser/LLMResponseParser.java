package com.apiorbit.lovableclone.llm.parser;
...
public class LLMResponseParser {
    ...
    public List<ChatEvent> parseChatEvents(String fullResponse, ChatMessage parentMessage) {
        ...
        while (matcher.find()) {
            ...
            log.info("First content: {}", matcher.group(1).toLowerCase());
            log.info("tagName: {}, attributes: {}", tagName, attributes);
            ...
            switch (tagName) {
                case "message" -> builder.type(ChatEventEnum.MESSAGE);
                case "file" -> {
                    builder.type(ChatEventEnum.FILE_EDIT);
                    builder.filePath(attrMap.get("path")); // Required for files
                }
                case "tool" -> {
                    builder.type(ChatEventEnum.TOOL_LOG);
